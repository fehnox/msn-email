...
function loadCredentials() {
  if (!fs.existsSync(CREDENTIALS_PATH)) throw new Error('credentials.json nao encontrado!');
...
function createOAuthClient() {
  ...
  return new google.auth.OAuth2(creds.client_id, creds.client_secret, 'http://localhost:3000/oauth2callback');
}

async function authenticate(tokenFile = 'token.json') {
  ...
  return new Promise((resolve, reject) => {
    const server = http.createServer((req, res) => {
      ...
      if (qs.code) {
        ...
        res.end('<html><body style="font-family:sans-serif;text-align:center;padding:3rem;background:#0d2137;color:white"><h2>MSN Mail autenticado!</h2><p>Pode fechar esta janela.</p></body></html>');
        server.close();
        resolve(qs.code);
      } else {
        res.writeHead(400); res.end('Erro'); server.close(); reject(new Error('Codigo nao recebido'));
      }
    });
    server.listen(3000);
...
async function fetchEmails(auth, { maxResults = 20, query = 'is:inbox' } = {}) {
  ...
  try {
    ...
    const emails = await Promise.all(messages.map(msg => fetchEmailDetail(gmail, msg.id)));
...
async function fetchEmailDetail(gmail, messageId) {
  try {
    ...
    return { id: messageId, from: fromName, email: fromEmail, subject, preview, body, account: 'gmail', time: timeStr, date: dateStr, unread: isUnread, raw: date };
  } catch (err) {
...
function extractBody(payload) {
  ...
  if (payload.parts) {
    ...
    if (textPart?.body?.data) return '<pre>' + decodeBase64(textPart.body.data) + '</pre>';
