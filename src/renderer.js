...
function renderEmailList() {
  ...
  document.getElementById('panelTitle').textContent =
    ...
    currentFilter === 'all' ? 'Todos os emails' : 'Gmail';
  if (emails.length === 0) {
    list.innerHTML = '<div style="padding:2rem;text-align:center;color:#6a8090;font-size:0.82rem;">' +
      (allEmails.length === 0 ? 'Carregando emails...' : 'Nenhum email encontrado') + '</div>';
    return;
  }
  list.innerHTML = '';
  emails.forEach(email => {
    ...
    item.innerHTML =
      ...
      '<div class="email-meta">' +
        ...
        '<div class="email-account-badge badge-gmail">Gmail</div>' +
      '</div>';
  ...
  preview.innerHTML =
    '<div class="preview-header">' +
      ...
      '<div class="preview-meta">' +
        '<div class="preview-meta-row"><span class="preview-meta-label">De:</span><span class="preview-meta-value">' + email.from + ' &lt;' + email.email + '&gt;</span></div>' +
        '<div class="preview-meta-row"><span class="preview-meta-label">Conta:</span><span class="preview-meta-value">Gmail</span></div>' +
        '<div class="preview-meta-row"><span class="preview-meta-label">Data:</span><span class="preview-meta-value">' + email.date + ' · ' + email.time + '</span></div>' +
      '</div>' +
    '</div>' +
...
document.getElementById('modalConfirm').addEventListener('click', () => {
  ...
  if (!email || !email.includes('@gmail.com')) {
    alert('Digite um email Gmail valido!');
    return;
  }
  if (modalOverlay) modalOverlay.classList.remove('open');
...
window.electronAPI.onEmailsLoaded((emails) => {
  ...
  updateStatus(emails.length + ' emails carregados' + (unreadCount > 0 ? ' · ' + unreadCount + ' nao lidos' : ''));
});
